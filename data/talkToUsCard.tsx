export const talkToUsCardList = [
    {
      imageSrc: "/static/images/pages/home/employee-1.jpg",
      title: "Building MVP?",
      description:
        "Looking to build fast and reduce your time to market? Drop me a message. Together we can find the core set of features to gather user information and iteratively create the best possible product.",
      buttonText: "Email Karl",
    },
    {
      imageSrc: "/static/images/pages/home/employee-2.jpg",
      title: "A Simple Conversation?",
      description:
        "Looking to build fast and reduce your time to market? Drop me a message. Together we can find the core set of features to gather user information and iteratively create the best possible product.",
      buttonText: "Email Arvind",
    },
    {
      imageSrc: "/static/images/pages/home/employee-3.jpg",
      title: "Non IT?",
      description:
        "Looking to build fast and reduce your time to market? Drop me a message. Together we can find the core set of features to gather user information and iteratively create the best possible product.",
      buttonText: "Email Nicholas",
    },
  ];
